...
@partial(jnp.vectorize, signature='(n),(n,n)->()')
def log_gaussian_density_from_cov_inv(x: Array, cov_inv: Array) -> float:
    """
    Evaluate log of gaussian density with zero mean and an inverse covariance matrix `cov_inv`.
    Parameters
    ----------
...
def outer(x: Array, y: Array):
    return jnp.outer(x, y)


@filter_jit
def log_sum_exp(log_weights: Array) -> float:
    """
...
@filter_jit
def log_mean_exp(log_weights: Array) -> float:
    """
...
@filter_jit
def uniform_spacings(uni: Array):
...
@filter_jit
def normalize_log_weights(log_weights: Array):
...
@filter_jit
def resample(indices: tuple[Array, Array], particles: Array) -> Array:
...
@filter_jit
def _prepare_weights(log_weights: Array) -> tuple[Array, int, int]:
    """
...
@filter_jit
def _resample(particles: Array, flat_indices: Array,
...
@filter_jit
def systematic_or_stratified(particles: Array, log_weights: Array,
                             uniform: Union[float, Array]) -> Array:
...
@filter_jit
def multinomial(particles: Array, log_weights: Array, uniform: float) -> Array:
    """
...
def resample_particle_history(particle_history: Array,
                              log_weight_history: Array,
                              uniform_history: Array,
                              resampling_method: str = 'systematic_or_stratified') \
        -> Array:
    """
    Resamples particles in a history of particles.

    Parameters
    ----------
    particle_history: Array
        The history of particles of shape (T, N_devices, N_samples_per_devices, N_state)
    log_weight_history: Array
        The history of log weights of shape (T, N_devices, N_samples_per_devices)
    uniform_history: Array
        The history of uniform random numbers used in resampling
    resampling_method: str, optional
        The resampling method to use, one of:
        - 'systematic'
        - 'stratified'
        - 'systematic_or_stratified'
        - 'multinomial'
        Default is 'systematic_or_stratified'

    Returns
    -------
    Array
        The resampled particle history of shape (T, N_devices, N_samples_per_devices, N_state)
    """
